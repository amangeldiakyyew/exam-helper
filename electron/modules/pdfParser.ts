...
export function prepareStudentInfo(classStudents: Record<string, StudentInfo>): StudentInfoMap {
  ...
  try {
    ...
    for (const [fullName, info] of Object.entries(studentInfo)) {
      ...
      if (nameParts.length === 2) {
        ...
        patterns.push(createPattern([nameParts[nameParts.length - 1], ...nameParts.slice(0, -1)]));
      } else {
